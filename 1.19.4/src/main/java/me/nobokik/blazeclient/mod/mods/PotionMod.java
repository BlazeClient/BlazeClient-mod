    ...
    public void render() {
        ...
        if(mc.player != null) {
            ...
            for (StatusEffectInstance effect : effects) {
                String name = Text.translatable(effect.getEffectType().getTranslationKey()).getString();
                if (textShadow.isEnabled()) {
                    ...
                    ImGui.pushStyleColor(ImGuiCol.Text, c[0], c[1], c[2], c[3]);
                    UI.shadowText(StatusEffectUtil.durationToString(effect, 1).getString(), 32, c[0], c[1], c[2], c[3]);
                    ImGui.popStyleColor(1);
                } else {
                    c = text.getColor().getFloatColor();
                    ImGui.pushStyleColor(ImGuiCol.Text, c[0], c[1], c[2], c[3]);
                    ...
                    c = durationText.getColor().getFloatColor();
                    ImGui.pushStyleColor(ImGuiCol.Text, c[0], c[1], c[2], c[3]);
                    ImGui.text(StatusEffectUtil.durationToString(effect, 1).getString());
                    ImGui.popStyleColor(1);
                }
            }
        } else {
